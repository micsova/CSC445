...
public class TFTPServer implements Runnable {
    ...
    public TFTPServer(File serverReadDirectory, File serverWriteDirectory, ServerMode mode) throws IOException {
        this(serverReadDirectory, serverWriteDirectory, DEFAULT_TFTP_PORT, mode, null, null, false);
    }
    ...
    public TFTPServer(File serverReadDirectory, File serverWriteDirectory, int port, ServerMode mode, PrintStream log,
                      PrintStream errorLog, boolean w) throws IOException {
        port_ = port;
        mode_ = mode;
     ...
     * @throws IOException if the server directory is invalid or does not exist.
     */
    public TFTPServer(File serverReadDirectory, File serverWriteDirectory, int port, InetAddress localaddr,
     ...
     * @throws IOException if the server directory is invalid or does not exist.
     */
    public TFTPServer(File serverReadDirectory, File serverWriteDirectory, int port, NetworkInterface localiface,
                      ServerMode mode, PrintStream log, PrintStream errorLog) throws IOException {
        mode_ = mode;
        port_ = port;
    ...
    public void run() {
        try {
            ...
            shutdownServer = true; //Set this to true to the launching thread can check to see if it started
    ...
    public class TFTPTransfer implements Runnable {
        ...
        public void run() {
            try {
                ...
                transferTftp_.open(true);

                if (tftpPacket_ instanceof TFTPReadRequestPacket) {
                    if (window) {
                        handleWindowRead(((TFTPReadRequestPacket) tftpPacket_));
                    } else {
                        handleStandardRead(((TFTPReadRequestPacket) tftpPacket_));
                    }
        ...
        */
        private void handleStandardRead(TFTPReadRequestPacket trrp) throws IOException, TFTPPacketException {
            InputStream is = null;
            InetAddress tftpAddress = trrp.getAddress();
            int tftpPort = trrp.getPort();
            try {
                if (mode_ == ServerMode.PUT_ONLY) {
                    transferTftp_.bufferedSend(new TFTPErrorPacket(tftpAddress, tftpPort,
                ...
                if (trrp.getMode() == Assignment2.tftp.TFTP.NETASCII_MODE) {
                ...
                while (readLength == TFTPDataPacket.MAX_DATA_LENGTH && !shutdownTransfer) {
                    if (sendNext) {
                        ...
                        System.out.println("Sending packet " + block + " to " + tftpAddress +
                                " on port " + tftpPort);
                        lastSentData = new TFTPDataPacket(tftpAddress, tftpPort, block, temp, 0,
                                readLength);
                    ...
                    while (!shutdownTransfer && (answer == null || !answer.getAddress().equals(tftpAddress) ||
                            answer.getPort() != tftpPort)) {
                        // listen for an answer.
                        if (answer != null) {
                            // The answer that we got didn't come from the
                            // expected source, fire back an error, and continue listening.
                        ...
                        try {
                            ...
                            // didn't get an ack for this data. need to resend it.timeoutCount++;
                            System.out.println("Resending packet " + block + " to " +
                                    lastSentData.getAddress());
        ...
        private void handleWindowRead(TFTPReadRequestPacket trrp) throws IOException, TFTPPacketException {
            InputStream is = null;
            InetAddress tftpAddress = trrp.getAddress();
            int tftpPort = trrp.getPort();
            try {
                ...
                try {
                    is = new BufferedInputStream(new FileInputStream(buildSafeFile(serverReadDirectory_, trrp.getFilename(),
                            false)));
                } catch (FileNotFoundException e) {
                    transferTftp_.bufferedSend(new TFTPErrorPacket(tftpAddress, tftpPort,
                            TFTPErrorPacket.FILE_NOT_FOUND, e.getMessage()));
                ...
                TFTPDataPacket[] lastSentData = new TFTPDataPacket[10];
                boolean notReceived = false;

                //Send the first n packets or the whole file if it's smaller than that.
                for (int i = 0; (i < lastSentData.length - 1) && (readLength == TFTPDataPacket.MAX_DATA_LENGTH) &&
                        !shutdownTransfer; i++) {
                    readLength = is.read(temp);
                    if (readLength == -1) {
                        readLength = 0;
                    ...
                    lastSentData[i] = new TFTPDataPacket(tftpAddress, tftpPort, block, temp,
                            0, readLength);
                    sendData(transferTftp_, lastSentData[i]); // send the data
                    block++;
                }
                ...
                while (readLength == TFTPDataPacket.MAX_DATA_LENGTH && !shutdownTransfer) {
                    if (sendNext) {
                        ...
                        if (notReceived) {
                            resend--;
                            if (resend == 0) {
                                sendData(transferTftp_, lastSentData[0]);
                                notReceived = false;
                                resend = 9;
                            }
                        }

                        lastSentData[lastSentData.length - 1] = new TFTPDataPacket(tftpAddress, tftpPort,
                                block, temp, 0, readLength);
                        sendData(transferTftp_, lastSentData[lastSentData.length - 1]); // send the data
                    }
                    ...
                    while (!shutdownTransfer && (answer == null || !answer.getAddress().equals(tftpAddress) ||
                        ...
                        try {
                            ...
                            // didn't get an ack for this data. need to resend it.timeoutCount++;
                            System.out.println("Resending block " + lastSentData[0].getBlockNumber());
                            sendData(transferTftp_, lastSentData[0]);
                            continue;
                        }
                    }
                    ...
                    if (answer == null || !(answer instanceof TFTPAckPacket)) {
                        ...
                        // once we get here, we know we have an answer packet from the correct host.
                        TFTPAckPacket ack = (TFTPAckPacket) answer;
                        if (ack.getBlockNumber() == lastSentData[0].getBlockNumber()) {
                            for (int i = 1; i < lastSentData.length; i++) {
                                lastSentData[i - 1] = lastSentData[i];
                            }
                            lastSentData[lastSentData.length - 1] = null;
                            // send the next block
                            block++;
                            if (block > 65535) {
                                // wrap the block number
                            ...
                            for (int i = 0; i < lastSentData.length; i++) {
                                if (lastSentData[i].getBlockNumber() == ack.getBlockNumber()) {
                                    for (int j = i + 1; j < lastSentData.length; j++) {
                                        lastSentData[j - 1] = lastSentData[j];
                                        lastSentData[j] = null;
                                    }
                                    break;
                                }
                ...
                for (; 0 < remaining;) {
                    ...
                    if (answer == null || !(answer instanceof TFTPAckPacket)) {
                        if (!shutdownTransfer) {
                            System.out.println("Unexpected response from tftp client during transfer (" + answer +
                                    ").  Transfer aborted.");
                        }
                        break;
                    } else {
                        ...
                        TFTPAckPacket ack = (TFTPAckPacket) answer;
                        if (ack.getBlockNumber() == lastSentData[0].getBlockNumber()) {
                            for (int j = 1; j < lastSentData.length; j++) {
                                lastSentData[j - 1] = lastSentData[j];
                            }
                            lastSentData[lastSentData.length - 1] = null;
                            notReceived = false;
                        } else {
                            for (int j = 0; j < lastSentData.length; j++) {
                                if (lastSentData[j].getBlockNumber() == ack.getBlockNumber()) {
                                    for (int k = j + 1; k < lastSentData.length; k++) {
                                        if (lastSentData[k] != null) {
                                            lastSentData[k - 1] = lastSentData[k];
                                        } else {
                                            break;
                                        }
                                    }
        ...
        private void handleWrite(TFTPWriteRequestPacket twrp) throws IOException, TFTPPacketException {
            ...
            try {
                ...
                try {
                    ...
                    if (twrp.getMode() == Assignment2.tftp.TFTP.NETASCII_MODE) {
